...
static KAUtilDelegateImpl *kaUtilDelegate;
...
void doShouldSDKInitForCurrentLocation(const char *appIDChar, const char *channelChar) {
    NSString *appID = [[NSString alloc] initWithUTF8String:appIDChar];
    NSString *channel = [[NSString alloc] initWithUTF8String:channelChar];
    if (kaUtilDelegate == nil) {
        kaUtilDelegate = [[KAUtilDelegateImpl alloc] init];
    }
    [[KAUtil sharedInstance] shouldSDKInitiateForCurrentLocaleWithAppId:appID channel:channel withDelegate:kaUtilDelegate];
}
